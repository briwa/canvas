...
export function start(section) {
  ...
  const countInput = section.querySelector('[data-el="count"]');
  const loopInput = section.querySelector('[data-el="loop"]');
  const restart = section.querySelector('[data-el="restart"]');
  const resetButton = section.querySelector('[data-el="reset-stats"]');
  ...
  for (const key of [
    ...
    readout[key] = section.querySelector(`[data-el="${key}"]`);
  }
  ...
  function rebuild() {
    ...
    const cards = makeCards(Number(countInput.value));
  ...
  return () => {
    ...
    restart.removeEventListener('click', rebuild);
    resetButton.removeEventListener('click', resetStats);
    countInput.removeEventListener('change', rebuild);

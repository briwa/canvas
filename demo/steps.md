...
const { Scene, layer, arc, circle, line, rect, forever, step, tween, wait, sequence, repeat, easeInOutSine, linear } = Canvas;
...
function equalizer(x, y, count = 9, w = 16, gap = 6, h = 150) {
  ...
  return layer([
    floor,
    layer(bars, repeat(tween(() => ({ y0: base - 12 - Math.random() * (h - 12) }), { duration: 240, ease: easeInOutSine }))),
  ]);
}

function circling(around, { rx, ry = rx, period }) {
  return forever((s) => {
...
function scribbling({ duration, from, to, y: middle }) {
  return step({
    duration,
    start(s) {
      const drawn = [];
...
function scribble(from, to, y, count = 32) {
  ...
  return layer(
    strokes,
    repeat(sequence(scribbling({ duration: 2600, from, to, y }), wait(900), tween({ alpha: 0 }, { duration: 500 }))),
  );
}
